...
outfile = s.fs.getsyspath(f'trial_averages_raw.mp4')
...
obj.data = s.ach_mov_raw.split(1).mean('trial')
for i in range(obj.data.sizes['time']):
    obj.data[i] = np.fliplr(np.rot90(obj.data[i]))
items.append(obj)
print('got ach')
