...
def make_mean_mov(s: Session) -> None:
    ...
    with h5py.File(s.fs.getsyspath('trials.h5'), 'r') as f:
        ...
        ach = xr.DataArray(f['ca'][:], dims=('trial', 'time', 'y', 'x'))
    ca = ca.mean('trial')
    ach = ach.mean('trial')
...
with h5py.File(s.fs.getsyspath('mean_mov.h5'), 'r') as f:
    mov_all = xr.DataArray(f['ca'][:], dims=('time', 'y', 'x'))
...
fig.savefig('figures/M150/retinotopic_map_M150_ca.png')
fig.savefig('figures/M150/retinotopic_map_M150_ca.eps')
# plt.show()
...
fig.tight_layout(pad=0.15)
fig.savefig('figures/M150/retinotopic_map_with_patches_M150_ca.png')
fig.savefig('figures/M150/retinotopic_map_with_patches_M150_ca.eps')
# plt.show()
...
# ax.set_yticks([])
fig.tight_layout(pad=0.15)
fig.savefig('figures/M150/retinotopic_map_traces_M150_ca.png')
fig.savefig('figures/M150/retinotopic_map_traces_M150_ca.eps')
# plt.show()
