...
def plot_fov():
    ...
    plt.show()
    PLOTDIR = Path.home() / "plots/grant update"
    savepath = PLOTDIR / "gach_fov.pdf"
    fig.savefig(savepath)


def plot_mean_trace():
    s = open_session('58470-1', '2022-12-20', '1', fs=-1)
    ...
    annotate_onsets(ax, arr.coords['event'], skip_first=True)
    xticklabels = [r'0.0', r'5.0']
    ax.set_xticklabels(xticklabels)
    ax.set_xlabel('sec.')
    plt.show()

    PLOTDIR = Path.home() / "plots/grant update"
    savepath = PLOTDIR / "mean_gach_trace.pdf"
    fig.savefig(savepath)
