...
for s in sessions:
    ...
    for i in range(spikes.sizes['roi']):
        ...
        sp = spikes.isel(roi=i)
        n_chunks = sp.sizes['time'] // 8
        sp = sp.isel(time=slice(0, n_chunks * 8))

        # grouped mean/std
        arr = sp.data.reshape(n_chunks, 8)
        means = arr.mean(axis=1)
        baselines.append(means.mean())
        stds.append(means.std())


baselines = np.array(baselines)
stds = np.array(stds)
thresh = baselines + 2 * stds
