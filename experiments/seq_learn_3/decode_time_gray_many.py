...
for day in (0, 5):
    ...
    for iter in range(n_iters):
        ...
        # split groups
        all_trial_inds = np.arange(1200)
        group_1_trial_inds = np.random.choice(all_trial_inds, 600, replace=False)
        ...
        dirpath = Path(__file__).parent / f'decode_time/gray_data/day{day}'
        dirpath.mkdir(exist_ok=True, parents=True)
        np.save(dirpath / f'{iter}.npy', cm)
