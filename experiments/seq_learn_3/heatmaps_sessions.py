...
def plot_heatmaps_for_session(
    s: Session,
    lpad: Optional[int] = None,
    rpad: Optional[int] = 24,
    roi_filter: str = 'all',
    show: bool = True,
    save: PathLike = 'analysis/heatmap.png',
) -> "matplotlib.figure.Figure":

    # -----------------------------------------------------------------------------#

    sessions = SessionGroup([s])
    ...
    fig.suptitle(s.mouse + f' day {s.attrs["day"]}')
    fig.tight_layout(pad=5)
    if show:
        plt.show()
    if save:
        savepath = s.fs.getsyspath(save)
